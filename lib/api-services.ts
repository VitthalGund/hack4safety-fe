...
export const apiService = {
  ...
  async fetchPerformanceRanking(entityType: "io" | "pp" | "unit", limit = 10) {
    return apiClient.get(`/analytics/performance/ranking?entity_type=${entityType}&limit=${limit}`)
  },
  ...
  // Case Search
  async searchCases(query: string, filters?: Record<string, string>, skip = 0, limit = 20) {
    const params = new URLSearchParams({ q: query, skip: skip.toString(), limit: limit.toString() })
    Object.entries(filters || {}).forEach(([key, value]) => {
      if (value) params.append(key, value)
    })
...
export async function fetchConvictionRate(groupBy: string, filters?: Record<string, string>) {
  return apiService.fetchConvictionRate(groupBy, filters)
}

export async function fetchTrends(period: "monthly" | "yearly", natureOfOffence?: string) {
  return apiService.fetchTrends(period, natureOfOffence)
}

export async function fetchPerformanceRanking(entityType: "io" | "pp" | "unit", limit = 10) {
...
export async function searchCases(query: string, filters?: Record<string, string>, skip = 0, limit = 20) {
